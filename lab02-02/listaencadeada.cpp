...
bool ListaEncadeada::checa_palindromo(){
    ...
    //ir nos dois sentidos comparando
    //se ficar diferente, retorna false
    //se terminar (ponteiros serem os mesmo ou se cruzarem)
    node_t *inicio = primeiro;
    node_t *fim = ultimo;

    while((inicio != fim) && (inicio->anterior != fim)){
        if(inicio->elemento != fim->elemento)
            return false;
...
void ListaEncadeada::remove_consecutivos(){
    ...
    node_t *compare = primeiro;
    node_t *finder = primeiro->proximo;
    int position = 1;
    while(finder != nullptr){
        while(finder->elemento == compare->elemento ) {
            finder = finder->proximo;
            remover_iesimo(position);
            if (finder->proximo == nullptr)
                return;
        }
        compare = finder;
        finder = compare->proximo;
        position++;
    }
